        ...
        {items.map((color) => {
          ...
          return (
            ...
            >
              <button
                onClick={() => {
                  if (color.available !== false && !isGolden) {
                    onSelectColor(color.id);
                    onClose();
                  }
                }}
                disabled={color.available === false || isGolden}
                  ...
                  {(type === "custom" || type === "mixed") && (
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-sm text-amber-400">
                        Tiempo de entrega: {color.leadTime}
                      </span>
                    </div>
                  )}
                </div>
          ...
          <div className="relative min-h-screen flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-6 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <ColorSection
                  title="Colores en Stock"
                  items={colors.stock}
                  type="stock"
                />
                <ColorSection
                  title="Colores Sobre Pedido"
                  items={colors.custom}
                  type="custom"
                />
                <ColorSection
                  title="Acabados Mixtos"
                  items={colors.mixed}
                  type="mixed"
                />
              </div>
